...
const schemas = {
  ...
  CreateFoodDto: object({ room_code: code, name: str(200,1), quantity: str(200), compartment: comp, container_tag: str(200), shelf_life_days: { type: 'integer', minimum: 0, maximum: 365 }, storage_path: { ...nullable(str(8192)), description: 'An owned, staged storage_path from POST /api/photos. photo_url is never client-settable.' }, notes: nullable(str(2000)), created_by: { ...str(), description: 'Ignored; actor comes from the verified session.' } }, ['room_code','name','compartment','shelf_life_days'], { additionalProperties: false }),
  UpdateFoodDto: object({ name: str(200,1), quantity: str(200), compartment: comp, container_tag: str(200), expiry_date: date, notes: nullable(str(2000)), storage_path: { ...nullable(str(8192)), description: 'An owned, staged storage_path from POST /api/photos, or null to remove the photo. photo_url is never client-settable.' } }, [], { minProperties: 1, additionalProperties: false }),
...
const source = { type: 'string', enum: ['gemini-3.1-flash-lite','heuristic'] };
...
endpoint('post','/api/photos','PhotoResult',{input:'PhotoRequest',status:201,parameters:[{name:'Idempotency-Key',in:'header',required:false,schema:str(200,1)}]});
endpoint('delete','/api/photos','Success',{input:object({storage_path:str()},['storage_path'],{additionalProperties:false})});
