...
Realtime credentials are room-scoped, short-lived and checked by RLS; configuration absent means unavailable/degraded polling, with no connected-mode latency guarantee. Per the operator-authorized F5 revision (2026-09-04), successful food/shopping changes target <=5 seconds from the source confirmation action to visible peer update when both apps are foregrounded, online on a stable network and connected to synchronization. This includes source API and peer rendering time. Record failures separately; never count rejected or unsaved operations as successful synchronization. This replaces the former <500ms requirement without changing API shapes or room authorization. On reconnect fetch full authoritative snapshots including empty and history. Offline reads may be stale; mutations require server success unless a tested outbox exists. No room recreation or cache snapshot POST replay.
