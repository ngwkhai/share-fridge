...
test('actual realtime lifecycle refreshes token, recreates failed channels, and cleans timers/listeners/stale callbacks',async()=>{
  ...
  let stop;
  try{
    stop=createRealtimeSubscription('721021',{refresh:async()=>reads++,transport:mode=>modes.push(mode),error:()=>errors++,delta:value=>deltas.push(value)},{config:{url:'https://fixture.supabase.co',anonKey:'public-fixture'},clientFactory:factory,getToken:async()=>({token:`room-token-${++credentials}`,expires_at:new Date(Date.now()+300000).toISOString()})});
    await flush();assert.equal(clients.length,1);const firstChannel=clients[0].getChannels()[0];assert.deepEqual(firstChannel.changes.map(x=>x.filter.event),['INSERT','UPDATE','changed','delta']);assert.ok(firstChannel.changes.slice(0,2).every(x=>x.filter.table==='room_sync_versions'&&x.filter.filter==='room_code=eq.721021'));assert.equal(firstChannel.privateConfig?.config?.private,true);
    firstChannel.callback('SUBSCRIBED');await flush();assert.equal(modes.at(-1),'connected');assert.equal(reads,1);
    firstChannel.changes[3].callback({payload:{resource:'food',operation:'delete',id:'food-a',room_code:'721021'}});assert.equal(deltas.length,1);
    assert.equal(await stop.publish({resource:'shopping',operation:'delete',id:'shop-a',room_code:'721021'}),true);assert.equal(firstChannel.sent.event,'delta');
    ...
    const readsBeforeStop=reads;stop.stop();await flush();assert.equal(timers.size,0);assert.equal(win.listeners.size,0);assert.equal(doc.listeners.size,0);
    ...
    stop?.stop();globalThis.setTimeout=originals.setTimeout;globalThis.clearTimeout=originals.clearTimeout;globalThis.setInterval=originals.setInterval;globalThis.clearInterval=originals.clearInterval;
