...
no advance. Read `BUILDFLOW.md` once for the method; THESE rules govern every session here.

**Stack:** React 18 + Vite + TypeScript + Tailwind CSS + PWA (`vite-plugin-pwa`) on Frontend; Supabase PostgreSQL + Edge API / OpenAPI spec at `/api/openapi.json` for live contract validation; Google Gemini 2.5 Flash API for AI services (Voice & Recipe).
...
7. **Playbooks** (`playbooks/`): paid-for stack knowledge. Before building a card that
   ...
   until the lesson is filed. More stacks → more playbooks. This project starts with an
   empty `playbooks/` — the first entry gets written the first time a card pays for one.

## Standard card sequence

1. **Scaffold + CI/CD** — repo, deploy pipeline, `/healthz` on a public URL. The scaffold
   picks the spec-serving mechanism (`[FILL]`: e.g. FastAPI: free at `/docs` +
   `/openapi.json`; other stacks: choose the generator in the ADR) — API docs are
   plumbing, not a later feature.
2. **Vertical slice** — one contract endpoint + one ugly page calling it, deployed.
   From this card on, the live spec page is LIVE on the deployed URL and shows the endpoint.
3. **Backend cards** — one endpoint-group per card, built TO the contract.
   **Spec lands WITH the API**: every backend card's verify includes "the card's
   endpoints appear in the live spec with correct request/response schemas" — its
...
4. **Contract-test card** — automated suite hitting EVERY contract endpoint with edge +
   ...
   If the framework serves a machine-readable spec (e.g. OpenAPI: `/openapi.json`), the
   suite ALSO asserts every `flow/05-contract.md` endpoint exists in the live spec with
   matching shapes — planning contract and runtime spec must never drift.
